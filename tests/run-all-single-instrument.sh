#!/bin/bash
# Full test run — single instrumentation call, all 50 classes at once
# No force-stop between classes (that destroys instrumentation registration)
...
# Build the -e class arguments for all 50 test classes (excluding A2UIPlaygroundYogaSmokeTest
# which is in the wrong package, and base/helper classes)
CLASSES=(
...
# Build class argument: comma-separated
CLASS_ARG=$(IFS=,; echo "${CLASSES[*]}")

OUTPUT_FILE="$RESULT_DIR/FULL_RUN.txt"

echo "=========================================="
echo "AGenUI Full Test Run (single instrumentation)"
echo "Device: $DEVICE"
echo "Classes: ${#CLASSES[@]}"
echo "Started: $(date)"
echo "=========================================="

# Run all classes in one instrumentation call
